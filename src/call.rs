...
impl SignedCallSignalV1 {
    /// Verifies the signature, time bounds, and shape, returning the
    /// domain-separated hash of the canonical signal.
    pub fn verify(&self, now_ms: i64, max_skew_ms: i64) -> anyhow::Result<CallSignalHash> {
...
mod tests {
    ...
    fn all_variants_sign_and_verify() {
        ...
        for signal in [
            ...
            assert!(signed.verify(1_000, 0).is_ok());
        }
    }
    ...
    fn sign_verify_roundtrip() {
        ...
        assert_eq!(signed.verify(1_000, 0).unwrap(), expected_hash);
        ...
        assert_eq!(decoded.verify(1_000, 0).unwrap(), expected_hash);
    }
    ...
    fn tamper_detection_on_every_field() {
        ...
        let baseline = signed.verify(1_000, 0).unwrap();
        ...
        assert_eq!(signed.verify(1_000, 0).unwrap(), baseline);
    }
    ...
    fn rejects_wrong_signer() {
        ...
        assert!(forged.verify(1_000, 0).is_err());
        ...
        assert!(signed_by_other.verify(1_000, 0).is_ok());
    }
    ...
    fn rejects_expired_signals() {
        ...
        assert!(signed.verify(1_999, 0).is_ok());
        assert!(signed.verify(2_000, 0).is_err());
        assert!(signed.verify(10_000, 0).is_err());
    }
    ...
    fn rejects_oversized_duration_and_future_issued() {
        ...
        assert!(signed.verify(1_000, 0).is_err());
        assert!(signed.verify(1_000, 4_000).is_ok());
    }
    ...
    fn revision_and_expiry_are_bound() {
        ...
        assert_ne!(
            signal_rev5.verify(1_000, 0).unwrap(),
            signal_rev6.verify(1_000, 0).unwrap()
        );
        ...
        assert!(tampered_rev.verify(1_000, 0).is_err());
        ...
        assert_ne!(
            signal_rev5.verify(1_000, 0).unwrap(),
            signal_later.verify(1_000, 0).unwrap()
        );
        ...
        assert!(tampered_expiry.verify(1_000, 0).is_err());
        ...
        assert!(signal_rev5.verify(2_500, 0).is_err());
        assert!(signal_later.verify(2_500, 0).is_ok());
    }
}
